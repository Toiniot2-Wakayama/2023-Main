...
void refreshBall8(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        ballX += ball[i*4] * cos(radians(45*i + 90));
        ballY += ball[i*4] * sin(radians(45*i + 90));
    }
    // atan2関数は -π ~ π までの範囲しかないから度数法に直した後180°=πradを足す
    ballAngle = degrees(atan2(ballY, ballX)) + 180;
    return;
}
...
void refreshBall16(void) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        ballX += cos(radians(45*i + 90));
        ballY += sin(radians(45*i + 90));
    }
    ballAngle = degrees(atan2(ballY, ballX)) + 180;
    return;
}
